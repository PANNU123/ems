...
class AcademicTypeController extends Controller
{
    public function index(Request $request){
        ...
        return view('admin.pages.academic_type.index');
    }
    ...
    public function trashed(Request $request){
        if ($request->ajax()) {
            $data = AcademicType::onlyTrashed()->get();
            return DataTables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($data) {
                    $btn = '<a href="javascript:voide(0)" class="btn btn-dark btn-sm "><i data-id="'.$data->id.'" class="fas fa-trash-restore trashedAcademic"></i></a>';
        ...
        return view('admin.pages.academic_type.trashed');
    }

    public function undoTrashed($id){
        $record = AcademicType::onlyTrashed()->where('id',$id)->first();
        $record->restore();
        return response()->json(['success' => true]);
    }
    public function restoreAll(){
        AcademicType::onlyTrashed()->restore();
        return redirect()->route('backend.academic.type.index');
    }
    ...
    public function permanentDeleteAll(){
        AcademicType::onlyTrashed()->forceDelete();
        return redirect()->route('backend.academic.type.index');
    }
