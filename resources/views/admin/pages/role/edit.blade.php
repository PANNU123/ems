...
@section('content')
    @include('admin.includes.breadcrumb',['breadcrumb' => 'Create Role'])
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-check">
                                                    <label><input class="form-check-input" type="checkbox"
                                                                  for="checkpermission" id="{{ $i }}Management"
                                                                  onclick="checkPermissionByGroup('role-{{ $i }}-management-checkbox', this)"
                                                                  value="{{ $group->group_name}}" {{ App\Models\User::roleHasPermissions($role, $permission) ? 'checked' : '' }}><span>{{$group->group_name}}</span></label>
                                                </div>
                                            </div>
                                            <div class="col-md-9 ms-auto role-{{ $i }}-management-checkbox">
                                                ...
                                                <div class="mt-4 mt-lg-0">
                                                    @foreach ($permissions as $item )
                                                        <div class="form-check">
                                                            <label><input class="form-check-input" type="checkbox"
                                                                          for="checkpermission{{$item->id}}"
                                                                          id="checkpermission{{$item->id}}"
                                                                          name="permissions[]"
                                                                          value="{{$item->name}}" {{ $role->hasPermissionTo($item->name) ? 'checked' : '' }}><span>{{$item->name}}</span></label>
                                                        </div>
                                                        @php  $j++; @endphp
                                                    @endforeach
                                                </div>
